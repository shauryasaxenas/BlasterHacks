...
fn render_plan<B: Backend>(f: &mut Frame<B>, app: &mut App, layout_chunk: Rect) {
    let plan = Paragraph::new("Place study/work plan generated by groq here.")
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title("Study Plan")
...
fn render_links<B: Backend>(f: &mut Frame<B>, app: &mut App, layout_chunk: Rect) {
    if let Some(i) = app.assignments_state.selected() {
        let table = List::new(app.data.assignments[i].relevant_links.into_iter().map(|link| {ListItem::new(link)}).collect::<Vec<_>>())
            .block(
                Block::default()
                .borders(Borders::ALL)
                .title("Links")
